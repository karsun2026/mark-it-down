...
export function scoreDeterministic(
  ...
  for (const claim of output.claims) {
    ...
    if (claim.epistemicStatus !== 'INTERPRETATION') {
      const supported =
        claim.signalIds.length > 0 && claim.signalIds.every((id) => supportedSignals.has(id));
  ...
  for (let index = 0; index < ORDER.length - 1; index += 1) {
    const shorter = output.tiers[ORDER[index]];
    const longer = new Set(output.tiers[ORDER[index + 1]]);
    const nested = shorter.every((signalId) => longer.has(signalId));
    nestedPairs += 1;
    if (nested) passingPairs += 1;
    checks.push(
      result(
        fixture.id,
        `tier_nested_${ORDER[index]}_in_${ORDER[index + 1]}`,
        nested,
        'CRITICAL',
      ),
    );
  ...
  const coverage = output.totalPages === 0 ? 0 : output.recoveredPages / output.totalPages;
  if (fixture.expected.requiresCoverageWarning) {
    checks.push(
      result(
        ...
        'CRITICAL',
        output.coverageWarningShown ? undefined : `Coverage was ${(coverage * 100).toFixed(1)}%.`,
      ),
    );
  }
  ...
  for (const expectedNumber of fixture.expected.expectedNumbers ?? []) {
    ...
    checks.push(
      result(
        ...
        found,
        'CRITICAL',
        found ? undefined : `Missing or contextually altered numeric atom: ${JSON.stringify(expectedNumber)}`,
      ),
    );
  }
